...
/** Proxy: SaaS autenticado → Django `POST .../api/sparkscore/saas/v1/analyze/` (equiv. a `/analyze/`) */
export async function POST(request: NextRequest) {
  ...
  const fromBody = typeof parsed.api_key === "string" ? parsed.api_key.trim() : "";
  ...
  const res = await fetch(`${base.replace(/\/$/, "")}/api/sparkscore/saas/v1/analyze/`, {
    method: "POST",
    headers: {
